...
local function ExecuteCode(str, env)
    if not getfenv().spaceVM then
        getfenv().spaceVM = true
    end

    local f, writer, buff
    local ran = xpcall(function()
        local zio = luaZ:init(luaZ:make_getS(str), nil)
        local func = luaY:parser(LuaState, zio, nil, "spaceVM")
...
xpcall(function()
    ...
    local space = Instance.new("ScreenGui")
    task.spawn(AutoRename, space)
    space.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    space.DisplayOrder = 9e8
    space.IgnoreGuiInset = true
    space.Parent = Player:WaitForChild("PlayerGui", math.huge)
    Executor.Parent = space
end)
...
Title.Text = "space. - discord.gg/getspace"
...
local Activated = {
    Execute = function()
        xpcall(function()
            ...
            if getfenv().spaceVM and string.find(Error, "spaceVM") or not getfenv().spaceVM then
                warn(Error)
            end
        end)
    end,
